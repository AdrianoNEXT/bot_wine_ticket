...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (interaction.isButton() && interaction.customId === "start_ticket1" || interaction.isButton() && interaction.customId === "start_ticket7") {
        ...
        if (interaction.isModalSubmit() && interaction.customId === "modal_ticket1") {
            ...
            const channel = await interaction.guild.channels.create({
                ...
                type: 0,
                parent: config.ticket.category_id1,
                permissionOverwrites: [
                    ...
                    {
                        id: config.ticket.support_role1,
                        allow: ["ViewChannel", "SendMessages", "AttachFiles", "AddReactions"]
                    }
                ],
            })
            ...
            channel.send({
                content: `||${interaction.user} - ${interaction.guild.roles.cache.get(config.ticket.support_role1)}||`,
                embeds: [
                    new Discord.EmbedBuilder()
                ...
                components: [new Discord.ActionRowBuilder()
                    .addComponents(
                        new Discord.ButtonBuilder()
                            .setCustomId("close_ticket1")
                            .setEmoji("🔒")
                            .setLabel("Fechar")
                            .setStyle(4),
                        new Discord.ButtonBuilder()
                            .setCustomId("painel_member1")
                            .setEmoji("👨‍💼")
                            .setLabel("Painel membro")
                            .setStyle(3),
                        new Discord.ButtonBuilder()
                            .setCustomId("painel_staff1")
                            .setEmoji("💼")
                            .setLabel("Painel staff")
                            .setStyle(1),
                        new Discord.ButtonBuilder()
                            .setCustomId("painel_sale1")
                            .setEmoji("🛒")
        ...
        if (interaction.isButton() && interaction.customId === "close_ticket1") {
            ...
            if (interaction.user.id !== user.id && !interaction.member.roles.cache.get(config.ticket.support_role1)) return interaction.reply({
                embeds: [
                    new Discord.EmbedBuilder()
                        .setColor(config.embeds_color.embed_error)
            ...
            interaction.update({
                ...
                components: [
                    new Discord.ActionRowBuilder()
                        .addComponents(
                            new Discord.ButtonBuilder()
                                .setCustomId("open_ticket1")
                                .setEmoji("🔓")
                                .setLabel("Abrir")
                                .setStyle(4),
                            new Discord.ButtonBuilder()
                                .setCustomId("delete_ticket1")
                                .setEmoji("🗑️")
                                .setLabel("Deletar")
                                .setStyle(4),
                            new Discord.ButtonBuilder()
        ...
        } else if (interaction.isButton() && interaction.customId === "open_ticket1") {
            const ticket = await db.get(`ticket_${interaction.channel.id}`);
        ...
        } else if (interaction.isButton() && interaction.customId === "painel_member1") {
            const ticket = await db.get(`ticket_${interaction.channel.id}`);
                    ...
                    new Discord.ActionRowBuilder()
                        .addComponents(
                            new Discord.StringSelectMenuBuilder()
                                .setCustomId('options_member1')
                                .setPlaceholder('Escolha uma opção!')
                                .addOptions(
                                    { label: '➕ Criar call', value: `create_call1` },
                                    { label: '🗑️ Deletar call', value: `delete_call1` },
                                    { label: '👤 Adicionar usuário', value: `add_user1` },
                                    { label: '🗑️ Remover usuário', value: `remove_user1` },
                                    { label: '💾 Salvar logs', value: `transcript1` }
                                )
                        )
                ],
                ephemeral: true
            })
        } else if (interaction.isStringSelectMenu() && interaction.customId === "options_member1") {
            const ticket = await db.get(`ticket_${interaction.channel.id}`);
            const user = await interaction.guild.members.cache.get(ticket.owner_id)

            const option = interaction.values[0];

            if (option === "create_call1") {
                ...
                const channel = await interaction.guild.channels.create({
                    ...
                    type: 2,
                    parent: config.ticket.category_call_id1,
                    permissionOverwrites: [
                        {
                            id: interaction.guild.id,
                            deny: ["ViewChannel"],
        ...
        } else if (interaction.isButton() && interaction.customId === "painel_staff1") {
            if (!interaction.member.roles.cache.get(config.ticket.support_role1)) return interaction.reply({
                embeds: [
                    new Discord.EmbedBuilder()
                        .setColor(config.embeds_color.embed_error)
            ...
            interaction.reply({
                ...
                components: [
                    new Discord.ActionRowBuilder()
                        .addComponents(
                            new Discord.StringSelectMenuBuilder()
                                .setCustomId('options_staff1')
                                .setPlaceholder('Escolha uma opção!')
                                .addOptions(
                                    { label: '🔔 Notificar usuário', value: `notify_user1` },
                                    { label: '💾 Salvar logs', value: `transcript1` },
                                    { label: '🗑️ Deletar ticket', value: `delete_ticket1` },
                                )
                        )
                ],
                ephemeral: true
            })
        } else if (interaction.isStringSelectMenu() && interaction.customId === "options_staff1") {
            const ticket = await db.get(`ticket_${interaction.channel.id}`);
            const user = await interaction.guild.members.cache.get(ticket.owner_id)

            const option = interaction.values[0];

            if (option === "notify_user1") {
                await user.send({
                    embeds: [
                        new Discord.EmbedBuilder()
                            .setColor(config.embeds_color.embed_invisible)
                ...
                const channel_send = interaction.guild.channels.cache.get(config.ticket.channel_logs1)
        ...
        } else if (interaction.isButton() && interaction.customId === "painel_sale1") {
            if (!interaction.member.roles.cache.has(config.ticket.support_role1)) return interaction.reply({
            ...
            interaction.reply({
                ...
                components: [
                    new Discord.ActionRowBuilder()
                        .addComponents(
                            new Discord.StringSelectMenuBuilder()
                                .setCustomId('options_sales1')
                                .setPlaceholder('Escolha uma opção!')
                                .addOptions(
                                    { label: '⭐ pix', value: `pix1` },
                                    { label: '💾 qrcode', value: `qrcode1` },
                                )
                        )
                ],
                ephemeral: true
            })
        } else if (interaction.isStringSelectMenu() && interaction.customId === "options_sales1") {
            const option = interaction.values[0];
        ...
        } else if (interaction.isButton() && interaction.customId === "delete_ticket1") {
            const ticket = await db.get(`ticket_${interaction.channel.id}`);
